...
def update_state_and_reward(timestamp, cur_time, botorch_hv, cur_step_cost,
    ...
    print("s_batch", s_batch, "a_batch", a_batch, "r_batch", r_batch)
    print(aaa)
...
def main_loop(args, agent, rl_model, timestamp, cur_time, budget):
    ...
    while cur_time.value < budget:
        ...
        # Set GPU allocation
        vanillaGPUsNums = 0 ####max(int(args.GPU_nums * vanilla_rate), 1)
        ...
        # Run oneshot and vanilla processes
        run_processes(None, VanillaNas, vanillaGPUsNums, None,
                      worker_budget, gpu_queue, cur_step_cost, timestamp,
                      cur_time, path, args) #OneshotNas #oneshotGPUsNums
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iter', type=int, default=3, help='total iterations') #38
    parser.add_argument('--sample_num', type=int, default=5, help='sample numsbers per iteration')
    parser.add_argument('--runs', type=int, default=2, help='total runs') #5
    ...
    parser.add_argument('--queueCapacity', type=int, default=1, help='maximum length of the gpu queue')
    parser.add_argument('--GPU_nums', type=int, default=1, help='total numbers of GPUs')
    parser.add_argument('--budget', type=int, default=4, help='time budget') #80
    parser.add_argument('--run', type=int, default=4, help='current run time') #0
    ...
    for i in range(len(carbon_trace)): #range(carbon_trace[0])
    ...
    lamoo_sampler = LamooSampler(problem=VanillaNas, nums_samples=1) # OneshotNas

    # Initialize MCTS agent
    agent = MCTS(lb=f.bounds[0].cpu().data.numpy(), ub=f.bounds[1].cpu().data.numpy(), dims=f.dim, ninits=10, args=args,
                  run=args.run, oneshot=None, vanilla=VanillaNas) #OneshotNas
