...
def load_data():
    ...
    try:
        return pd.read_csv('US-CAL-CISO_2021_hourly.csv')   #pd.read_csv('US-CAL-CISO.csv') #../electricity-maps-2020-2022-all-regions-marginal-average/
    except FileNotFoundError:
        return pd.read_csv('../../electricity-maps-2020-2022-all-regions-marginal-average/US-CAL-CISO.csv')

def get_carbon_trace(data, start_index=0):
    # Extract carbon intensity data starting from a specific index
    return data['Carbon Intensity gCO₂eq/kWh (direct)'].tolist()[start_index:] #data['carbon_intensity_avg'].tolist()[start_index:]
