...
if __name__ == "__main__":
    ...
    for j in range(9):
      ...
      for i in range(300):
        ...
        pred = 'output/NAS-BENCH-201-4/simplifies/architectures/' + str(i).zfill(6) + '-FULL.pth'
        xdata  = torch.load(pred)

        odata  = xdata['full']['all_results'][('cifar10', 777)]
        result1 = ResultsCount.create_from_state_dict( odata )
        scores[float(result1.get_train()['loss'])] = (acc,acc_test, pred)#(acc1, acc)#we use loss to store MGM score
      ...
      print(top_acc[0][1])
      config = torch.load(top_acc[0][2])['full']['all_results'][('cifar10', 777)]['arch_config']
      all_outputs.append(config)
      with open('predicted_networks.npy', 'wb') as f:
